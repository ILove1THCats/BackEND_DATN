 ...
 */
//TODO: Chưa ổn định
export const createPlace = async (name, description, address, type_id, region_id, created_by, geom // ví dụ: 'POINT(105.123 21.456)'
) => {
    const result = await pool.query(`INSERT INTO places 
        (name, description, address, type_id, region_id, created_by, geom)
     VALUES ($1, $2, $3, $4, $5, $6, ST_GeomFromText($7, 4326))
     RETURNING *`, [name, description, address, type_id, region_id, created_by, geom]);
 ...
 */
//TODO: Chưa ổn định
export const updatePlace = async (id, name, description, address, type_id, region_id, geom) => {
    const result = await pool.query(`UPDATE places
     SET name = COALESCE($2, name),
         ...
         updated_at = NOW()
     WHERE place_id = $1
     RETURNING *`, [id, name, description, address, type_id, region_id, geom]);
    return result.rows[0] || null;
};
/**
 * Xoá địa điểm
...
export const deletePlace = async (id) => {
    const result = await pool.query(`DELETE FROM places WHERE place_id = $1 RETURNING *`, [id]);
    return result.rows[0] || null;
};
/**
 ...
 */
//TODO: Chưa ổn định
export const getPlaceWithAverageRating = async (placeId) => {
