...
export const getSpecificPlace = async (amenity: string): Promise<Place[]> => {
  ...
  `SELECT ST_X(geom) AS long, ST_Y(geom) AS lat, operator, name FROM places where amenity = $1`, [amenity]);
...
export const nearbyPlace = async (lat: number, lon: number, amenity: string, radius: number): Promise<Place[]>  => {
    const result = await pool.query(`
