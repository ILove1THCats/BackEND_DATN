...
export const getLikePlace = async(placeId: number) => {
  const { rows } = await pool.query(
    `SELECT COUNT(*) AS total FROM user_favorites where place_id = $1;`, [placeId]);
...
export const reviewInsert = async (placeid:number, userid:number, rating:number, comment:string) => {
  const client = await pool.connect();
